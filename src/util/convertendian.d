...
T readReverse(T, InputStream)(ref InputStream stream) {
	ubyte b;
	stream.read(b);
	T value = b;
...
T readNormal(T, InputStream)(ref InputStream stream) {
	T value;
...
void writeReverse(T, OutputStream)(ref OutputStream stream, T value) {
	for (size_t shift = 0; shift < T.sizeof * 8; shift += 8) {
		stream.write(cast(ubyte)((value >>> shift) & 0xFF));
	}
}
/// Writes bytes to stream.
void writeNormal(T, OutputStream)(ref OutputStream stream, T value) {
